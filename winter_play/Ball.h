...
void BoardCrushingJudge(int i) {
    if (MyBall[i].Speed_y > 0 &&
        IsRect(MyBall[i].Ball_rect.x, MyBall[i].Ball_rect.y, player1.BoardDesRect.x, player1.BoardDesRect.y,
               MyBall[i].Ball_rect.w, MyBall[i].Ball_rect.w, Length, thickness)) {
        MyBall[i].Speed_y *= -1;
        MyBall[i].Ball_ele = player1.ele;
        if (fabs(MyBall[i].Speed_x) < 6 || MyBall[i].Speed_x * player1.direct < 0) {
            MyBall[i].Speed_x += player1.direct * 2.8;
...
void CreateBall() {
    ...
    MyBall[ExistingBalls].Ball_rect.x = player1.BoardDesRect.x + BoardSet.x / 2;
    ...
    MyBall[ExistingBalls].Speed_y = -8;
...
void BallPlaceUpdate(int i) {
    ...
    if (MyBall[i].collide == true) {
        MyBall[i].Ball_ele = player1.ele;
        MyBall[i].Ball_rect.x = player1.BoardDesRect.x + BoardSet.x / 2;
...
void shoot(int i) {
    if (MyBall[i].collide == true) {
        ...
        MyBall[i].Speed_x = player1.direct * rand() % 10 / 3.0;
    }
}
...
void Need_To_Add_Ball() {
    if (MyScore / 30 > ExistingBalls + DisappearBalls - 1) {
        CreateBall();
        shoot(ExistingBalls - 1);
    }
